    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      snapshot.forEach((docSnap) => {
        ...
        list.push({
          ...
          dataReagendamento: d.dataReagendamento || '',
      ...
      // 19. TRATAMENTO DE ERROS CLAROS - handleFirestoreError
      try {
...
export async function savePedido(pedido: Omit<Pedido, "id"> & { id?: string }): Promise<string> {
  ...
  if (isFirebaseConfigured && db) {
    ...
    try {
      ...
      handleFirestoreError(err, pedido.id ? OperationType.UPDATE : OperationType.CREATE, `pedidos/${pedido.id || 'new'}`);
    }
  } else {
    // LocalStorage mode
    const list = getLocalStoragePedidos();
    ...
    if (pedido.id) {
      ...
      if (idx !== -1) {
        list[idx] = { ...list[idx], ...payload, id: finalId };
      }
    } else {
